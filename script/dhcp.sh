...
sed -e "s/\${IP}/'$IP'/" -e "s/\${NETMASK}/$NETMASK/" config/interfaces > $DEFAULT_INTERFACE_CONFIG
...
IP=$(echo $IP| egrep '[[:digit:]]{1,3}\.[[:digit:]]{1,3}\.[[:digit:]]{1,3}' | cut -d. -f1-3 | awk '{ print $2 }' | cut -d: -f2)
# We fill our template with our IP range
